 ...
 *
 */
public class MergeSort_withComparator {
	
	public static void sort(Comparator c, Comparable [] array, Comparable [] auxArray, int low, int high){
		
		if(high <= low) return;
		int mid = low + (high-low)/2;
		sort(c, array, auxArray, low, mid);
		sort(c, array, auxArray, mid+1, high);
		merge(c, array, auxArray, low, mid, high);
	}
	
	private static void merge (Comparator c, Comparable [] array, Comparable [] auxArray, int low, int mid, int high) {
		...
		for(int x=low; x<=high; x++){
			...
			if(i>mid){
				...
				array[x] = auxArray[i++];
			}else if(SortUtil.lessThan(c, auxArray[j], auxArray[i])){
				array[x] = auxArray[j++];
			}else{
				array[x] = auxArray[i++];
			}
