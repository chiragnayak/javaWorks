 ...
 *
 */
public class PriorityQ_MaxQ_usingBinaryHeap {
	...
	private void swim(int k) {
		 ...
		 * 
		 * lessThan(array[k/2], array[k])
		 * 
		 * -> if parent is less than current node, then swap
		 * 
		 * */
		
		while(k > 1  && SortUtil.lessThan(array[k/2], array[k])){ // k/2 >= 1
	...
	/**
	 * Scenario : parent key becomes smaller than one (or both) ot its children's.
	 * 
	 * What to do ?
	 * exchange key in parent with the key in larger child. 
	 * 
	 * in other words :
	 * 	1) find which one of the two childeren in greater.
	 *  2) exchange with that greater child. 
	 *  3) Repeat until heap is restored. (both children are small)
	 *  
	 *  Why with greater child ?
	 *  A : 
	 * 
	 * 
	 * @param k
	...
	public void sink(int k){
		...
		while(2*k <= lastPosition){
			...
			if(j < lastPosition && SortUtil.lessThan(array[j], array[j+1])) j++;
			
			if(SortUtil.lessThan(array[j], array[k])) break;
